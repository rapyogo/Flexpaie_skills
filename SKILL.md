...
Ce document dÃ©crit l'intÃ©gration de FlexPaie au sein d'une architecture **Laravel utilisant Firestore** pour la persistance des donnÃ©es. C'est la mÃ©thode exacte utilisÃ©e dans le projet Viteat 9.2.

**Principe de base :**
1. **Initiation** : L'utilisateur lance le paiement. Le serveur crÃ©e un document dans `mobile_money_payments/{reference}` avec le statut `pending` et appelle l'API FlexPay.
2. **Polling / Callback** : Le client interroge l'Ã©tat via `flexpayStatus` et FlexPay envoie un callback asynchrone sur `flexpayCallback`. Ces deux processus mettent Ã  jour Firestore.
3. **Confirmation** : Une fois complÃ©tÃ©, `flexpayConfirm` valide le montant, la devise et l'identitÃ© avant de clÃ´turer le panier/la commande.

## Environnement & Configuration

Les clÃ©s ne sont pas hardcodÃ©es dans un fichier `.env`. Elles sont lues dynamiquement depuis Firestore dans la collection `settings/flexpay_settings` et sÃ©curisÃ©es en backend.
Certaines variables optionnelles peuvent Ãªtre prÃ©sentes dans `.env` :
...
Ce service s'occupe uniquement des appels cURL vers l'API FlexPay. Il accepte un constructeur dynamique pour lire les clÃ©s venant de Firestore.
...
## 2. ImplÃ©mentation du ContrÃ´leur (Firestore)

Les mÃ©thodes sont intÃ©grÃ©es dans le contrÃ´leur principal (ex: `SecurePaymentController`), et stockent l'Ã©tat dans Firestore `mobile_money_payments/{reference}` via un `FirebaseService` injectÃ© (`$this->fs`).

### 2.1. Initialisation (`flexpayInit`)
CrÃ©e un document Firestore "pending" avant d'appeler l'API FlexPay. Si succÃ¨s, ajoute l'`orderNumber`.
...
public function flexpayInit(Request $request)
{
    ...
    $uid = Auth::user()->uuid ?? ''; // Adaptation selon votre systÃ¨me de User
    ...
    // RÃ©cupÃ¨re les clÃ©s depuis Firestore "settings/flexpay_settings"
...
Permet au frontend de vÃ©rifier le statut. Interroge Firestore puis FlexPay si nÃ©cessaire, et corrige le statut Firestore.
...
public function flexpayStatus(Request $request)
{
    ...
    if (!$orderNumber && $reference) {
        ...
        if (($doc['status'] ?? '') === 'completed') {
            return response()->json(['status' => 'completed', 'message' => 'Paiement validÃ©']);
        }
    }
    ...
    if (in_array($status, ['0', 'completed', 'successful', 'success', 'approved'])) {
        ...
        return response()->json(['status' => 'completed', 'message' => 'Paiement rÃ©ussi']);
    }
...
FlexPay envoie une requÃªte POST sur ce webhook. Validation de l'IP appelante et mise Ã  jour Firestore idempotente.
*Note: Cette route doit Ãªtre exclue du middleware `VerifyCsrfToken`.*
...
### 2.4. Confirmation SÃ©curisÃ©e Finale (`flexpayConfirm`)
AprÃ¨s le succÃ¨s cÃ´tÃ© frontend, on confirme avec Firebase avant d'enregistrer l'achat dÃ©finitif (Panier/Wallet/Cart). VÃ©rification du montant, devise, et user_id pour Ã©viter la fraude.
...
public function flexpayConfirm(Request $request)
{
    ...
    if (($payment['status'] ?? '') !== 'completed') {
        return response()->json(['status' => false, 'message' => 'Paiement non complÃ©tÃ©.'], 422);
    }
    ...
    // Marquer l'ordre final comme payÃ©
    return response()->json([
        'status' => true,
