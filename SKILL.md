...
description: Guide d'architecture universel par Michel Bengana pour int�grer FlexPay.cd Mobile Money dans N'IMPORTE QUEL langage (PHP, Node.js, Python, Java...). Couvre les concepts techniques (Polling, Webhooks, S�curit�) avec des exemples en PHP, Laravel (Firestore) et Next.js.
---
...
>
> *Mon objectif est que cette comp�tence facilite l'int�gration de FlexPaie pour tous les d�veloppeurs, **peu importe le langage ou le framework qu'ils utilisent** (Node.js, Python, Java, Go, etc.). Ce document standardise l'architecture, les processus m�tier et les **termes techniques universels** (Initiation, Polling, Callback / Webhook, Idempotence). Les extraits de code en PHP, Laravel et Next.js servent de r�f�rence, mais la logique est transposable partout.*
## Overview
...
## Variante 3 : Package & Architecture Laravel Native Complète (Eloquent / MySQL / PostgreSQL)

Cette variante fournit une intégration native de type package Laravel (PSR-4 `FlexPay\Laravel\`), prête pour Laravel 10, 11 et 12 avec base de données relationnelle (MySQL, PostgreSQL, MariaDB).

Les fichiers sources complets sont disponibles dans [`examples/laravel/`](./examples/laravel/) ou directement à la racine du repository.

### 1. La Règle Critique du Statut API (`status: "2"`)

> [!CAUTION]
> **COMPRÉHENSION IMPÉRATIVE DU CODE `status: "2"` :**
> - **`status: "0"`** ➔ **Succès confirmé** (débité par l'opérateur).
> - **`status: "2"`** ➔ **En attente de code PIN** (`"Le paiement est en attente"`).
> - **`status: "1"`** ➔ **Échec réel** (solde insuffisant, rejet ou timeout).
>
> Traiter `status: "2"` comme une erreur avorte le polling à 3 secondes. L'utilisateur clique à répétition et les passerelles télécoms RDC (Airtel USSD / Vodacom M-Pesa) verrouillent la ligne pendant **15 minutes (900s)** ! Le polling doit toujours maintenir l'attente pendant au moins 180 secondes.

---

### 2. Détection Dynamique des 4 Opérateurs RDC (`OperatorDetector.php`)
...
{
    public const AIRTEL   = 'airtel';   // 097, 098, 099 (+24397, +24398, +24399)
    public const VODACOM  = 'vodacom';  // 081, 082, 083 (+24381, +24382, +24383)
    public const ORANGE   = 'orange';   // 080, 084, 085, 089 (+24380, +24384...)
    public const AFRICELL = 'africell'; // 090, 091 (+24390, +24391)

    public static function detect(string $phone): ?string;
    public static function normalize(string $phone): string; // Retourne 243xxxxxxxxx
...
### 3. Service Client HTTP (`FlexPayService.php`)

Gère l'appel à l'API REST v1 avec timeout et retries propres, et intègre un mode `SIMULATED` pour le développement local :
...
$result = FlexPay::initiateMobileMoney(
    ...
    amount: 15.00,
    currency: 'USD',
    callbackUrl: route('flexpay.callback')
);

// Vérification de statut (Polling)
$status = FlexPay::checkTransaction($orderNumber);
if ($status['is_completed']) {
    // Succès confirmé (status 0)
} elseif ($status['is_pending']) {
    // Toujours en attente du PIN (status 2) -> CONTINUER LE POLLING
}
```

---

### 4. Modèle Eloquent avec Transitions Atomiques (`FlexPayPayment.php`)

Évite les conditions de course (*race conditions*) entre le polling utilisateur et le webhook FlexPay :

```php
// Validation atomique : seule une transaction en attente peut être validée
$payment->markAsCompleted($channel, $payload);
```

---

### 5. Middleware de Sécurité IP Webhook (`VerifyFlexPayWebhookIp.php`)

Filtre les appels vers `POST /payments/flexpay/callback` en vérifiant la liste blanche officielle (`156.0.198.27`, `156.0.198.19`) en s'appuyant sur `TrustProxies` de Laravel pour une compatibilité parfaite avec Google Cloud Run, AWS ALB ou Nginx.

---

### 6. Composant Blade Interactif RDC

```html
<x-flexpay::payment-modal 
    :amount="$order->total" 
    :currency="$order->currency" 
    :reference="$order->reference" 
    :onSuccessUrl="route('orders.show', $order->id)" 
/>
```